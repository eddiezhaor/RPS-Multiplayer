  ...
  database.ref("player").on("value", function(snapshot) {
    ...
    if (numPlayers === 0) {
      ...
      if (snapshot.child("1").exists()) {
        ...
        <br>
        <br>
         <p>Wins:${snapshot.child("1").val().wins} Losses: ${
      ...
      if (snapshot.child("2").exists()) {
        $("#playerTwo").html(
         ...
         <br>
         <br>
         <p>Wins:${snapshot.child("2").val().wins} Losses: ${
    ...
    if (numPlayers === 2) {
      ...
      if (
        snapshot.child("2").val().choice === "" &&
        snapshot.child("1").val().choice !== ""
      ) {
        $("#choose").text("it is Your Turn!!!!");
        $(".spr").css("visibility", "visible");
      }
      ...
      if (Player1choice !== "" && Player2choice !== "" && counter1 !== 0) {
        ...
        setTimeout(function() {
          ...
          $(".join").text("it is Your Turn!!!!");
          $("#choose").text(
            `Waiting for ${$("#Player1Name").text()} to choose`
          );
          $(".spr").css("visibility", "hidden");
          Player1choice = "";
          Player2choice = "";
...
$("#start").on("click", function(event) {
  ...
  if (numPlayers === 0) {
    $(".name").prepend(
      `<p class="join">Waiting for another person to join</p>`
    );
    startPlayerOne();
  } else if (numPlayers === 1) {
    $(".name").prepend(
      `<p id="choose">Waiting for ${$("#Player1Name").text()} to choose</p>`
    );
...
database.ref("chat").on("value", function(snapshot) {
  ...
  if (snapshot.val().user !== "") {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      var messageHTML = "";
      messageHTML = `<div><b>${messages[aKey].user}</b>: ${
        snapshot.val()[aKey].message
      }</div>`;
